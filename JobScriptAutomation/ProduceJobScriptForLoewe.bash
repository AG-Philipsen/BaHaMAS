...
function ProduceJobscript_Loewe(){
    ...
    if [ $CLUSTER_NAME = "LOEWE" ]; then
        __static__AddToJobscriptFile "#SBATCH --partition=$LOEWE_PARTITION"
        if [[ "$LOEWE_PARTITION" == "parallel" ]]; then
            __static__AddToJobscriptFile "#SBATCH --constraint=$LOEWE_CONSTRAINT"
        fi
        if [[ "$LOEWE_NODE" != "unset" ]]; then
            __static__AddToJobscriptFile "#SBATCH -w $LOEWE_NODE"
        fi
