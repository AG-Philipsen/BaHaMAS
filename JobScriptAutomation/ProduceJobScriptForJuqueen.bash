function ProduceJobscript_Juqueen(){
    #This jobscript is for tmLQCD only!
    ...
    echo 'MASS='"'""$MASS""'"'' >> $JOBSCRIPT_GLOBALPATH
    echo 'NTIME='"'""$NTIME""'"'' >> $JOBSCRIPT_GLOBALPATH
    echo 'NSPACE='"'""$NSPACE""'"'' >> $JOBSCRIPT_GLOBALPATH
    echo 'BETA='"'""$BETA""'"'' >> $JOBSCRIPT_GLOBALPATH
    echo '# @ job_name         = '"$CHEMPOT_PREFIX""$CHEMPOT"'_'"$MASS_PREFIX""$MASS"'_'"$NTIME_PREFIX""$NTIME"'_'"$NSPACE_PREFIX""$NSPACE"'_'"$BETA_PREFIX""$BETA" >> $JOBSCRIPT_GLOBALPATH
    echo '# @ notify_user      = '"$USER_MAIL" >> $JOBSCRIPT_GLOBALPATH
    echo 'export THIS_JOB='"$JOBSCRIPT_PREFIX"'_'"$CHEMPOT_PREFIX""$CHEMPOT"'_'"$MASS_PREFIX"'${MASS}_'"$NTIME_PREFIX"'${NTIME}_'"$NSPACE_PREFIX"'${NSPACE}_'"$BETA_PREFIX"'${BETA}' >> $JOBSCRIPT_GLOBALPATH
    echo 'export SPECIFIC_PATH='"$SIMULATION_PATH"'/mui'"$CHEMPOT"'/'"$MASS_PREFIX"'${MASS}/'"$NTIME_PREFIX"'${NTIME}/'"$NSPACE_PREFIX"'${NSPACE}/'"$BETA_PREFIX"'${BETA}' >> $JOBSCRIPT_GLOBALPATH
