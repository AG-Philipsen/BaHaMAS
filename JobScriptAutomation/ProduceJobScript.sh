...
echo 'NTAU='"'""$NTAU""'"'' >> $JOBSCRIPT_GLOBALPATH
echo 'NSIGMA='"'""$NSPAT""'"'' >> $JOBSCRIPT_GLOBALPATH
echo 'BETA='"'""$BETA""'"'' >> $JOBSCRIPT_GLOBALPATH
echo '# @ job_name         = '"$CHEMPOT_PREFIX""$CHEMPOT"'_'"$KAPPA_PREFIX""$KAPPA"'_'"$NTAU_PREFIX""$NTAU"'_'"$NSPAT_PREFIX""$NSPAT"'_'"$BETA_PREFIX""$BETA" >> $JOBSCRIPT_GLOBALPATH
echo '# @ notify_user      = '"$USER_MAIL" >> $JOBSCRIPT_GLOBALPATH
echo 'export THIS_JOB='"$JOBSCRIPT_PREFIX"'_'"$CHEMPOT_PREFIX""$CHEMPOT"'_'"$KAPPA_PREFIX"'${KAPPA}_'"$NTAU_PREFIX"'${NTAU}_'"$NSPAT_PREFIX"'${NSIGMA}_'"$BETA_PREFIX"'${BETA}' >> $JOBSCRIPT_GLOBALPATH
echo 'export SPECIFIC_PATH='"$SIMULATION_GLOBALPATH"'/mui'"$CHEMPOT"'/'"$KAPPA_PREFIX"'${KAPPA}/'"$NTAU_PREFIX"'${NTAU}/'"$NSPAT_PREFIX"'${NSIGMA}/'"$BETA_PREFIX"'${BETA}' >> $JOBSCRIPT_GLOBALPATH
