...
if [ "$CLUSTER_NAME" = "JUQUEEN" ]; then
    ...
    echo 'export SPECIFIC_PATH='"$SIMULATION_GLOBALPATH"'/mui'"$CHEMPOT"'/'"$KAPPA_PREFIX"'${KAPPA}/'"$NTIME_PREFIX"'${NTIME}/'"$NSPACE_PREFIX"'${NSPACE}/'"$BETA_PREFIX"'${BETA}' >> $JOBSCRIPT_GLOBALPATH
    ...
    echo "#SBATCH --partition=parallel" >> $JOBSCRIPT_GLOBALPATH
    echo "" >> $JOBSCRIPT_GLOBALPATH
