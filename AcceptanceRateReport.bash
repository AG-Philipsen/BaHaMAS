...
function AcceptanceRateReport()
{
    ...
    for INDEX in "${!BETAVALUES_COPY[@]}"; do
        local OUTPUTFILE_GLOBALPATH=$WORK_DIR_WITH_BETAFOLDERS/$BHMAS_betaPrefix${BETAVALUES_COPY[$INDEX]}/$OUTPUT_FILENAME
        if [ ! -f $OUTPUTFILE_GLOBALPATH ]; then
            cecho lr "\n File " file "$OUTPUT_FILENAME" " not found in " dir "$WORK_DIR_WITH_BETAFOLDERS/$BHMAS_betaPrefix${BETAVALUES_COPY[$INDEX]}"\
    ...
    for BETA in ${BETAVALUES_COPY[@]}; do
        OUTPUTFILE_GLOBALPATH=$WORK_DIR_WITH_BETAFOLDERS/$BHMAS_betaPrefix$BETA/$OUTPUT_FILENAME
