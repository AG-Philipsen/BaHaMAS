...
function ProduceJobscript_SLURM(){
    ...
    if [ $CLUSTER_NAME = "LOEWE" ]; then
        __static__AddToJobscriptFile "#SBATCH --partition=$CLUSTER_PARTITION"
        if [[ "$CLUSTER_PARTITION" == "parallel" ]]; then
            __static__AddToJobscriptFile "#SBATCH --constraint=$CLUSTER_CONSTRAINT"
        fi
        if [ "$CLUSTER_NODE" != '' ]; then
            __static__AddToJobscriptFile "#SBATCH -w $CLUSTER_NODE"
        fi
